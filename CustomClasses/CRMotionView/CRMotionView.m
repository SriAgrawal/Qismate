...
- (void)commonInit
{
    ...
    _motionEnabled = YES;
    _zoomEnabled   = YES;
    // _scrollDragEnabled = YES;
    // _scrollBounceEnabled = YES;
    
    [self startMonitoring];
    //    // Tap gesture to open zoomable view
    //UIPinchGestureRecognizer *tapGesture = [[UIPinchGestureRecognizer alloc] initWithTarget:self action:@selector(handleTap:)];
...
-(void)addZoomView{
    ...
    self.zoomScrollView.zoomDelegate = self;
    [self addSubview:self.zoomScrollView];
    [self.zoomScrollView setBackgroundColor:[UIColor clearColor]];
    [self.zoomScrollView.imageView setHidden:YES];
}
