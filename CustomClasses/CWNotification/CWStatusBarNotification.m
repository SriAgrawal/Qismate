...
{
    CGFloat height;
    if (SYSTEM_VERSION_LESS_THAN(@"8.0") && [[UIApplication sharedApplication] statusBarOrientation]) {
...
- (CGFloat)getStatusBarHeight
{
 ...
 //   UIDeviceOrientationIsLandscape([UIApplication sharedApplication].statusBarOrientation)
    if (SYSTEM_VERSION_LESS_THAN(@"8.0") && [[UIDevice currentDevice] orientation]) {
...
//    NSTimer* myTimer = [NSTimer scheduledTimerWithTimeInterval: 0.5 target: self selector: @selector(loadFunction:) userInfo: nil repeats: YES];
}
-(void) loadFunction:(NSTimer*) t
{
        //write code to change the text of label or any other component from here...
        if ([self.notificationLabel.text isEqualToString: @"saving." ]) {
            self.notificationLabel.text = @"saving..";
        } else if ([self.notificationLabel.text isEqualToString: @"saving.." ]) {
            self.notificationLabel.text = @"saving...";
        } else if ([self.notificationLabel.text isEqualToString: @"saving..." ]) {
            self.notificationLabel.text = @"saving.";
        }
