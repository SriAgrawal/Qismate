...
+ (void)showImageView:(UIImageView *)avatarImageView {
    ...
    [background setBackgroundColor:[[UIColor blackColor] colorWithAlphaComponent:0.5]];
...
+ (void)showImageView:(UIImageView *)avatarImageView : (NSString *)imageType{
...
//    [newAvatarImageView addGestureRecognizer:longPressOnImage];
    ...
    [UIView animateWithDuration:0.3f
                     animations:^
     {
         [newAvatarImageView setFrame:CGRectMake(0, top, screenWidth, screenWidth)];
         [background setAlpha:1];
         if ([imageType isEqualToString:@"avtar"]) {
             
             newAvatarImageView.layer.cornerRadius = screenWidth/2;
             newAvatarImageView.clipsToBounds = YES;
         }
     }];
}
