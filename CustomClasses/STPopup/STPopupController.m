...
@interface STPopupView (STInternal)
...
@interface STPopupController () <UIViewControllerTransitioningDelegate, UIViewControllerAnimatedTransitioning, STPopupNavigationTouchEventDelegate,STPopupViewTouchEventDelegate>
{
...
//    _arrowDownImage = [[UIImageView alloc]initWithFrame:CGRectMake((windowWidth-20)/2, windowHeight-((windowHeight-380)/2), 20, 15)];
//    _arrowDownImage.image = [UIImage imageNamed:@"play"];
   // [backgroundView addSubview:_arrowDownImage];
...
- (void)layoutContainerView
{
    ...
    for(UIView *subview in topViewController.view.subviews){
        if([subview isKindOfClass:[STPopupView class]]){
            STPopupView *view = (STPopupView *)subview;
            [view setTouchEventDelegate:self];
        }
    }
}
...
{
    _containerView = [STPopupView new];
...
#pragma mark - STPopupViewTouchEventDelegate
...
- (void)popupView:(STPopupView *)navigationBar touchDidMoveWithOffset:(CGFloat)offset
{
    ...
    if (offset < 0)
    {
        [UIView
         ...
         animations:^{
             [_arrowDownImage setAlpha:0.0f];
         }];
    }else{
        NSLog(@"move Popup");
        [self.delegate onMovingPopup];
    }
...
- (void)popupView:(STPopupView *)navigationBar touchDidEndWithOffset:(CGFloat)offset
{
    ...
    else
    {
        [UIView
         ...
         animations:^{
             [_arrowDownImage setAlpha:1.0f];
         }];
        NSLog(@"Cancel end");
        [self.delegate didFailDismissPopup];
        [_containerView endEditing:YES];
        [UIView animateWithDuration:0.4 delay:0 usingSpringWithDamping:0.7 initialSpringVelocity:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
            _containerView.transform = CGAffineTransformIdentity;
        } completion:nil];
